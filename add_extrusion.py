################################################################################
## This script converts a G-code file (.gcode) with X and Y motion only, and  ##
## output a G-code file (.gcode) with added extrusion commands                ## 
...
def getCoordinate(input):
		...
		if tmp.count("F") < 1:
			yIndex = list(range(tmp.index("Y")+1, len(tmp)))
		else:
			yIndex = list(range(tmp.index("Y")+1, tmp.index("F")))
...
def main(args):
	...
	file = args.filename if args.filename else 'motion_only.gcode'
	...
	print("length of original G-code = %d" % len(content))
	looper = content.split('\n')
	print("number of lines in content = %d" % len(looper))
	counter = 0
	for line in looper:
		if "* SHAPE Nr:" in line:
			originLine = counter + 1
		elif "startFlag" in line:
			beginLine = counter + 1
	...
	for line in looper:
		if counter >= beginLine and counter <= endLine:
			try:
				...
				print(line)
			goToX, goToY = goTo[0], goTo[1]
			distance = math.sqrt((goToX - lastX)**2 + (goToY - lastY)**2)
	...
	if raise_amount:
		...
		clown += raiseCommand
		print('raise needle command activated:\n', raiseCommand)
